...
T = 1.0/co_params[((0,0), (1,1))]['gam']*3.0/5.0         # relaxation time constant
# dt = 1e-15
# T = 1e-12
...
resp_xs_spec_fft = fftp.fft2(resp_xs, planner_effort='FFTW_ESTIMATE')
# resp_xs_spec_fft = np.zeros((ts.size, ts.size), dtype=np.complex)
# for i in range(ts.size):
#     resp_xs_spec_fft[:, i] = fftp.fft2(resp_spec[:, i],
#                                        planner_effort='FFTW_ESTIMATE')

# *** Plot
fig_dict = vis.plot2d_im(freqs=(resp_xs_freqs_wn[imin:imax],
                                resp_xs_freqs_wn[imin:imax]),
                         spec2d=np.real(resp_xs_spec_fft[1:,1:]),
