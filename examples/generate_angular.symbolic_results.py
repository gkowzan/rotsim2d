...
rfactors_dict = {k: rfactorize(gfactors[k], symr.T00_exprs, coeffs=True) for k in gfactors.keys()}
rfactors_xxxx = {k: factor(powdenest(v.subs({phi: S(0), phj: S(0), phk: S(0), phl: S(0)}), force=True), deep=True)
                 for k, v in rfactors.items()}
rfactors_relative = {k: rfactorize(gfactors[k], symr.T00_exprs, relative=True) for k in gfactors.keys()}
rfactors_highj = {k: rfactorize(gfactors_highj[k], symr.T00_exprs) for k in gfactors_highj.keys()}
rfactors_highj_dict = {k: rfactorize(gfactors_highj[k], symr.T00_exprs, coeffs=True)
