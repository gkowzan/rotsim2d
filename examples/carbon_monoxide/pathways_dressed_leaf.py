...
pws = pw.gen_pathways(20, [np.pi/2, sopr, np.pi/2, np.pi/4], co_pops, [pw.remove_rephasing, pw.remove_interstates])
# pws = pw.gen_pathways(20, [0.0, np.arctan(2/np.tan(np.pi/4)), 0.0, np.pi/4], co_pops, [pw.remove_rephasing, pw.remove_interstates])
# pws = pw.gen_pathways(20, [0.0]*4, co_pops, [pw.remove_rephasing, pw.remove_interstates])
ll = pw.dress_pws(pws, sys_params)
pl = pw.peak_list(ll)
...
dls_nonreph = [pw.DressedLeaf(l, sys_params) for l in nonreph.leaves]
dls_by_js = pw.split_by_js(dls_nonreph)
